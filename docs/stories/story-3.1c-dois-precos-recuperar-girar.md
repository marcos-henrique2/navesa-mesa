...
- [x] **O Risk #4 SE MATERIALIZA — em `/repasses/anuncio`, e foi corrigido.** ⚠️ *Correção de uma nota anterior errada do `@dex-dev`, que afirmava o contrário. O erro foi procurar o **nome** do artefato citado na story ("badge do `/repasses`") em vez do **comportamento**; o `@quinn-qa` achou o comportamento.*
  - **Onde a lista `/repasses` está certa:** ela é só `RepassesLista`, **não** chama `classificarBadge` e não tem coluna de `valor_minimo`/`valor_compre_por` — as cores dela vêm de `classificarMargemVenda` e de `delta < 0`. **Continua limpa, e nenhuma query foi escrita pra ela.**
  - **Onde o risco morava:** `relatorio-anuncio.ts:211` — `if (it.valorMinimo != null && it.valorMinimo < it.custoReal) prejuizoLatente.push(it)`. É **o predicado exato da C16, sem qualificação**, alimentando um painel permanente (`RelatorioAnuncio.tsx:225-250`): caixa vermelha *"Prejuízo latente — N carro(s)"* com a placa em chip, montada sobre **todos** os carros `status='subido'` (`relatorio-anuncio.ts:161`). Do primeiro carro girado com `g > 6,6%` em diante, a placa dele ficaria ali listada como prejuízo **para sempre**.
  - **Corrigido:** `calcularAlertas` passa a receber o mapa de snapshots e **exclui do `prejuizoLatente` só a origem `decisao`**; `deriva` e `fora_do_sistema` seguem vermelhos. Omitir o mapa mantém o comportamento anterior byte a byte — é a rede de degradação quando a leitura da 030 falha.
  - **A query de lista do IN recuperou o consumidor:** `buscarUltimosSnapshotsPrecificacao(ids)`, uma query só sobre o `idx_rep_prec_repasse_recente`, reduzida a `Map<repasseId, SnapshotRecente>`.
  - **`prejuizoNoAnuncio`: intacto na camada de dados, MAS a tela mudou.** ⚠️ *Correção de uma nota anterior que dizia só "INTACTO" — verdadeiro no array, **falso na tela**.* O array `prejuizoNoAnuncio` **nunca é uma caixa própria**: ele vira o `Set piores` e só **realça placas dentro da caixa de `prejuizoLatente`** (`RelatorioAnuncio.tsx:222-250`). Antes da C16 o subset se sustentava (todo carro do `prejuizoNoAnuncio` estava também no `prejuizoLatente`); **depois da C16 não se sustenta mais**. Consequência prática: **um carro girado com `g > 11,97%` sai do `prejuizoLatente` por origem `decisao` e, com ele, SOME DO PAINEL** — mesmo continuando no array.
    **Decisão do Marcos (2026-08-12) — opção (b): some, e está certo que suma.** Racional dele: se girou sabendo que abre mão dos gastos, o compre-por baixo é consequência da **mesma** decisão, não surpresa que mereça alerta. **O sinal não sai da tela** — a linha do carro na tabela mantém o 🔴 via `it.cor`; o que sai é a caixa de "não deixe passar". Também **não ampliar** a C16 pro `PainelNegociacao` (`InteressadosCRM.tsx:612`), que tem o mesmo gatilho.
- [ ] **Imprecisão conhecida da C16, aberta pra decisão de produto** *(estava só no docstring de `origem-abaixo-do-custo.ts`; docstring é onde o implementador olha, DoD é onde a decisão de produto se reabre)*: aplicar **à mão** abaixo do custo no modo `recuperar_tudo` — permitido pela **AC19 da 3.1** — é classificado como **`deriva`**, embora não haja deriva nenhuma (o custo não se moveu). **A reação na tela está certa** (o vermelho original, que é o que se quer mostrar); o rótulo interno é que mente. `fora_do_sistema` seria pior: imprimiria *"Não há registro de decisão de preço para este valor"*, que é **frase falsa** — existe registro. A §12.8 **não enumera** esse caso. Fechar exige **quarta origem** = texto novo (`@uma-ux`) + emenda de ADR (`@aria-architect`). **Só vale abrir se a distinção precisar aparecer na tela**
- [ ] **Débito registrado (LOW, não consertar aqui):** os percentuais de régua na UI usam `.toFixed(1)` e saem com **ponto** ("106.6%"), enquanto a justificativa vinda do motor usa `toLocaleString("pt-BR")` e sai com **vírgula** ("106,6%"). As duas grafias **já conviviam antes desta fatia** (verificado em `0311cba`), e corrigir aqui violaria a não-regressão byte a byte que a C1/§5 exigem do estado colapsado. A 3.1c ampliou os usos de `.toFixed(1)` de 3 pra ~9. **Fatia de UI própria:** trocar `pctRegua` e a linha de razão do `PrecoCard` pra `toLocaleString("pt-BR")`
