...
export function HistoricoSnapshots() {
  ...
  return (
    <div className="space-y-6">
      ...
      {snapshots.length === 0 ? (
        <div className="rounded-2xl border border-dashed border-[var(--border-base)] bg-[var(--bg-surface)] p-10 text-center">
          <Camera className="mx-auto h-8 w-8 text-[var(--text-subtle)]" />
...
function Delta({ atual, anterior, invertido }: { atual: number; anterior: number; invertido?: boolean }) {
  ...
  if (Math.abs(diff) < 0.01) return <span className="inline-flex items-center gap-0.5 text-[var(--text-subtle)]"><Minus className="h-3 w-3" /></span>;
...
function CardEvol({ titulo, valor, delta }: { titulo: string; valor: string; delta: React.ReactNode }) {
  return (
    <div className="rounded-xl border border-[var(--border-soft)] bg-[var(--bg-surface)] p-4 shadow-[var(--shadow-sm)]">
      <p className="text-[10px] font-semibold uppercase tracking-wider text-[var(--text-muted)]">{titulo}</p>
      <p className="mt-1 text-lg font-bold tabular-nums text-[var(--text-strong)]">{valor}</p>
  ...
  return (
    <section>
      <h2 className="mb-3 flex items-center gap-2 text-base font-semibold text-[var(--text-strong)]">
        <TrendingUp className="h-4 w-4 text-[var(--brand-700)] dark:text-[var(--brand-300)]" /> Vendas ao longo do tempo
      </h2>
...
function SecaoEstoque({ snapshots }: { snapshots: Snapshot[] }) {
  ...
  return (
    <section>
      <h2 className="mb-3 flex items-center gap-2 text-base font-semibold text-[var(--text-strong)]">
        <TrendingDown className="h-4 w-4 text-[var(--brand-700)] dark:text-[var(--brand-300)]" /> Estoque ao longo do tempo (custo de fábrica)
      </h2>
      <div className="overflow-x-auto rounded-xl border border-[var(--border-soft)] bg-[var(--bg-surface)] shadow-[var(--shadow-sm)]">
        <table className="w-full text-sm">
          <thead className="bg-[var(--bg-muted)] text-left text-xs font-semibold uppercase tracking-wider text-[var(--text-muted)]">
          ...
          <tbody>
            {comEstoque.map((s) => (
              <tr key={s.id} className="border-t border-[var(--border-soft)] hover:bg-[var(--bg-muted)]">
                ...
                <td className="px-4 py-2 text-right tabular-nums text-[var(--text-body)]">{formatBRL(s.estoque!.custoTotal)}</td>
