...
/**
 * Todos os modos são relatórios do NBS: processam local e terminam num store do
 * navegador — nada aqui toca o Supabase.
 *
 * O sync por arquivo do Auto Avaliar (que GRAVA em produção) NÃO mora aqui: ele
 * vive em `/repasses/importar`, junto da importação por texto.
 */
...
export function UploadDropzone({ modo }: { modo: Modo }) {
  ...
  const { getRootProps, getInputProps, isDragActive } = useDropzone({
    ...
    maxFiles: 1,
    // 50MB — XLSX real do NBS é ~1-3MB; previne DoS local com arquivo gigante.
